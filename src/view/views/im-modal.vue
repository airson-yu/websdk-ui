<template>
  <Modal v-model="main_modal_show" class="sdk-modal sdk-user-modal sdk-user-main-modal" scrollable :title=uname
         :width=1000 :z-index=1000
         :mask="false" :footer-hide=true v-on:on-cancel=on_hide_modal v-on:on-visible-change=on_visible_change>
    <!--:width=510-->
      ...
      <Split v-model="im_split" min="150" max="300">
        ...
        <div v-if="im_target_type==1" slot="left" class="sdk-left">
          ...
          <div class="sdk-touch-div">
            <i-switch v-model="dcg_attached" :loading=dcg_attach_loading size="large" @on-change="reqCallPTT"
                      title="强呼"/>
          </div>
          <div class="sdk-left-bottom">
            <div class="sdk-oper-toggle-div">
              <div class="sdk-oper-toggle">
                <Icon v-show="oper_type==1" @click="toggleOperType" type="md-arrow-dropup"
                      class="sdk-oper-toggle-icon"/>
                <Icon v-show="oper_type==2" @click="toggleOperType" type="md-arrow-dropdown"
                      class="sdk-oper-toggle-icon"/>
              </div>
            </div>
            ...
            <div v-bind:key="item.uid" v-for="item in tg_mem_list">
              <div class="sdk-left-mem-item">
                ...
                <Dropdown v-show="video_conf_on" v-on:on-click=operateVideo
                  ...
                  <Icon v-show="item.video_ing" type="md-videocam" :size=22
                        class="sdk-left-mem-video sdk-icon-btn sdk-icon-btn-on"/>
                  <DropdownMenu slot="list">
                    ...
                    <DropdownItem v-show="!item.video_share_ing" :name="'shareVideo_'+item.uid" divided>分享视频
                    </DropdownItem>
                    <DropdownItem v-show="item.video_share_ing" :name="'stopShareVideo_'+item.uid" divided>停止分享
                    </DropdownItem>
                  </DropdownMenu>
                </Dropdown>
              </div>
              ...
              <div v-show="oper_panel==1" class="sdk-oper-panel">
                <div>
                  ...
                  <div class="sdk-oper-item" title="视频会商">
                    <Icon @click="toggleVideoConf" v-bind:class="{'icon-btn-on': video_conf_on}" type="ios-desktop"
                          class="sdk-icon-btn"/>
        ...
        <div slot="right" class="sdk-im-panel">
          ...
          <div id="sdk_im_div" class="sdk-im-div">
            <div v-for="item in im_list">
              ...
              <IMTextRight v-if="item.type === 'text_send'" :uname="my_name" :avatar="my_avatar" :ts="item.ts"
                           :target="item.target"
                           :content="item.content"></IMTextRight>
              <IMTextLeft v-if="item.type === 'text_receive'" :uname="item.uname" :avatar="item.avatar" :ts="item.ts"
                          :target="item.target"
                          :content="item.content"></IMTextLeft>
              <IMImageRight v-if="item.type === 'image_send'" :uname="my_name" :avatar="my_avatar" :ts="item.ts"
                            :target="item.target"
                            :url_small="item.url_small" :url_large="item.url_large"></IMImageRight>
              <IMImageLeft v-if="item.type === 'image_receive'" :uname="item.uname" :avatar="item.avatar" :ts="item.ts"
                           :target="item.target"
                           :url_small="item.url_small" :url_large="item.url_large"></IMImageLeft>
              <IMFileRight v-if="item.type === 'file_send'" :uname="my_name" :avatar="my_avatar" :ts="item.ts"
                           :target="item.target"
                           :content="item.content" :file_name="item.file_name"></IMFileRight>
              <IMFileLeft v-if="item.type === 'file_receive'" :uname="item.uname" :avatar="item.avatar" :ts="item.ts"
                          :target="item.target"
            ...
            <div v-show="ptt_receive_ing && im_target_type == 2">
              <IMVoiceLeft :uname="ptt_receive_uname" :avatar="ptt_receive_avatar" :init_playing="true"
                           :ts="ts_r"></IMVoiceLeft>
            </div>
